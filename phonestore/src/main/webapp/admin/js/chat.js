...
document.querySelectorAll("#listuserchat tr").forEach(row => {
    row.addEventListener("click", function() {
        console.log("User clicked!"); // Kiểm tra xem có log không
        let userNameElement = this.querySelector("td:nth-child(2)");
        if (userNameElement) {
            let userName = userNameElement.textContent.trim();
            console.log("Selected user:", userName); // Kiểm tra giá trị user
            document.getElementById("chatWithUser").textContent = "Chat với " + userName;
        }
    });
});
